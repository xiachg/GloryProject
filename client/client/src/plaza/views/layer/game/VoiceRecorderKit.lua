...
function VoiceRecorderLayer:removeRecorde()
    ...
    if RECORD_STATE then
        if not self.m_bCancelRecord then
            ...
            local tabBuffer = {AudioRecorder:getInstance():createSendBuffer()}
            local nBufferCount = #tabBuffer
            print("录音包 ==> ", nBufferCount)
            for i = 1, nBufferCount do
                local buffer = tabBuffer[i]
                if nil ~= buffer then
                    print(" 发送录音 ", i)
