...
--操作任务
function TaskLayer:onHandleTask(wTaskID, cbTaskStatus, item)
	self._scene:showPopWait()
...
function TaskLayer:onButtonClickedEvent(tag,sender)
	if tag == TaskLayer.BT_CELL then
        ...
        if nil ~= item then
            ...
            self:onHandleTask(item.wTaskID, item.cbTaskStatus, item)
        end		
	end
end
...
function TaskLayer:tableCellAtIndex(view, idx)	
	...
	if nil ~= btn then
        ...
        elseif item.cbTaskStatus == yl.TASK_STATUS_WAIT then
            if item.cbPlayerType == 2 then
                btn:loadTextureNormal("Task/bt_task_vip_take_0.png")
                btn:loadTexturePressed("Task/bt_task_vip_take_1.png")
                btn:loadTextureDisabled("Task/bt_task_vip_take_0.png")
            else
