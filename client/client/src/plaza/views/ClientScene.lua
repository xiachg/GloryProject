	...
	if 1 == #self:getApp()._gameList and yl.SINGLE_GAME_MODOLE then
		...
		if nil ~= entergame then
			...
			if MatchRoom and true == MatchRoom:getInstance():isCurrentGameOpenMatch(GlobalUserItem.nCurGameKind) then
				if PriRoom and false == PriRoom:getInstance():isCurrentGameOpenPri(GlobalUserItem.nCurGameKind) 
        			and 0 == GlobalUserItem.GetGameRoomCount(GlobalUserItem.nCurGameKind) then
        			self:onChangeShowMode(MatchRoom.LAYTAG.MATCH_ROOMLIST)
    			else
	...
	local checkInInfo = function(result, msg, subMessage)
		...
		if result == 1 then
			if false == GlobalUserItem.bTodayChecked then
				self:onChangeShowMode(yl.SCENE_CHECKIN, nil , function()
					-- 广告显示在签到界面之上
