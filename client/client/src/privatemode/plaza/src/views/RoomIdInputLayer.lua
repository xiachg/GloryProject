...
function RoomIdInputLayer:showLayer( var )
    ...
    if var then
        ...
        GlobalUserItem.szCopyRoomId = GlobalUserItem.szCopyRoomId or ""
        ...
        local enterRoomId = ""
        if 6 == string.len(copyId) then
            -- 复制id
            enterRoomId = copyId
        elseif "table" == type(joinGame) and "string" == type(joinGame["roomid"]) and 6 == string.len(joinGame["roomid"]) then
            -- 缓存暂离id
            enterRoomId = joinGame["roomid"]
        end

        if 6 == string.len(enterRoomId) then
            self.m_atlasRoomId:setString(enterRoomId)
