...
function RoomDetailLayer:ctor( tabDetail )
    ...
    -- 是否允许解散
    local enableDismiss = tabDetail.enableDismiss or false
    ...
    self.m_tabGradList = gradList
    local count = #gradList
    count = (count > 8) and 8 or count
...
function RoomDetailLayer:tableCellTouched(view, cell)
    
end

function RoomDetailLayer:cellSizeForTable(view, idx)
    return 0, 45
end

function RoomDetailLayer:numberOfCellsInTableView(view)
    return self.m_nCount
end

function RoomDetailLayer:tableCellAtIndex(view, idx)
