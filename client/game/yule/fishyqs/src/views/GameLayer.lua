...
function GameLayer:onSubSupply(databuffer )
  ...
  if supply.nSupplyType == g_var(cmd).SupplyType.EST_Gift then
    self._dataModel.m_secene.lPlayCurScore[1][supply.wChairID+1] = self._dataModel.m_secene.lPlayCurScore[1][supply.wChairID+1] + supply.lSupplyCount
    print("=============== onSubSupply ============",supply.wChairID,self._dataModel.m_secene.lPlayCurScore[1][supply.wChairID+1])
...
function GameLayer:onSubFishCatch2( databuffer )

  --print("捕捉鱼...........................................")
  local catchNum = math.floor(databuffer:getlen()/18)
  print("=========== catchNum   ===========",catchNum)
  if catchNum >= 1 then
    for i=1,catchNum do
      print("=========== catchNum index  ===========",i)
      while true do
        local catchData = ExternalFun.read_netdata(g_var(cmd).CMD_S_CatchFish,databuffer)
        if not self.m_cannonLayer  then
          local fish = self._dataModel.m_fishList[catchData.nFishIndex]
           ...
           cannonPos = CannonSprite.getPos(self._dataModel.m_reversal,cannonPos)
             ...
             if catchData.wChairID == self.m_nChairID then   --自己
                ...
                if catchData.nFishType ~= g_var(cmd).FishType_BaoXiang and catchData.nFishType ~= g_var(cmd).FishType_YuanBao then
                  self._dataModel.m_secene.lPlayCurScore[1][catchData.wChairID + 1] = self._dataModel.m_secene.lPlayCurScore[1][catchData.wChairID + 1] + catchData.lScoreCount
                  local cannon = self.m_cannonLayer:getCannoByPos(cannonPos + 1)
                  cannon:updateScore(self._dataModel.m_secene.lPlayCurScore[1][catchData.wChairID + 1]) 
                end  
                  --更新用户分数
                  ...
                  for k,v in pairs(self.m_cannonLayer._userList) do
                    ...
                    if item.dwUserID == userid  then
                      ...
                      if catchData.nFishType ~= g_var(cmd).FishType_BaoXiang and catchData.nFishType ~= g_var(cmd).FishType_YuanBao then
                            self._dataModel.m_secene.lPlayCurScore[1][item.wChairID+1] = self._dataModel.m_secene.lPlayCurScore[1][item.wChairID+1] + catchData.lScoreCount
                            self.m_cannonLayer:updateUserScore(self._dataModel.m_secene.lPlayCurScore[1][item.wChairID+1],cannonPos+1 )
                            break
                      end
                      break
