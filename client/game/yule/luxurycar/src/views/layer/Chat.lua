...
function Chat:tableCellAtIndex( view, idx )
	...
	if nil == cell then
		...
		--昵称
		local nick =  g_var(ClipText):createClipText(cc.size(245, 18),record[1].." :")
		...
		if record[2]._type == 0 then
			content =  cc.Label:createWithTTF(record[2]._content, "fonts/round_body.ttf", 19,cc.size(260,0))
		else
