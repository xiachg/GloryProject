...
function GameViewLayer:ctor(scene)
	...
	--加载资源
	self:loadResource()
    self.m_openCardTime = 0 -- 开牌总时间 包括 发牌+显示牌点+比较
...
function GameViewLayer:loadResource()
    ...
    self.m_timeLayout:setPosition(self.m_timeLayout:getPositionX(),self.m_timeLayout:getPositionY() - 70)
    --庄家背景框
...
--]]
    --玩家分数
    self.m_lUserMaxScore = cmd_table.lUserMaxScore -- 最大下注
    ...
    self.m_bEnableSysBanker = cmd_table.bEnableSysbanker --系统坐庄
    self.m_lAreaLimitScore = cmd_table.lAreaLimitScore --区域限制
    self.m_bGenreEducate = cmd_table.bGenreEducate --房间类型
    self.m_lAllJettonScore = cmd_table.lAllJettonScore[1] 	--全体总注
    self.m_lUserJettonScore = cmd_table.lUserJettonScore[1] --个人总注
    ...
    self:resetBankerInfo()
    self:resetSelfInfo()
    self.m_lSelfWinScore = cmd_table.lEndUserScore --玩家成绩
    for k,v in pairs(self.m_lUserJettonScore) do
        self.m_lUserAllJetton = self.m_lUserAllJetton + v
    end
    self.m_cbTableCardArray = cmd_table.cbTableCardArray --桌面扑克
    if self.m_cbGameStatus == Game_CMD.GAME_SCENE_JETTON then
        self:setJettonEnable(false)
        self:updateJettonList(self.m_lUserMaxScore - self.m_lUserAllJetton*MaxTimes)
...
function GameViewLayer:updataDataTime(time)
        if time > 0 then
            self.m_timeLayout:setVisible(true)
        end
        local icon = self.m_timeLayout:getChildByName("im_icon")
        ...
        time:setString(string.format("%02d", self.m_cbTimeLeave))
        if self.m_cbGameStatus == Game_CMD.GAME_SCENE_FREE then
            self.m_openCardTime = 0
            self.isBagen = false
            icon:loadTexture("im_time_free.png",UI_TEX_TYPE_PLIST)
            --content:loadTexture("txt_time_free.png", UI_TEX_TYPE_PLIST)
             content:loadTexture("sp_tip_1.png")
        elseif self.m_cbGameStatus == Game_CMD.GAME_SCENE_JETTON then
            self.m_openCardTime = 0
            self.isBagen = false
            icon:loadTexture("im_time_jetton.png",UI_TEX_TYPE_PLIST)
            --content:loadTexture("txt_time_jetton.png", UI_TEX_TYPE_PLIST)    
             content:loadTexture("sp_tip_2.png")
        elseif self.m_cbGameStatus == Game_CMD.GAME_SCENE_END then
            content:loadTexture("sp_tip_3.png")
            if self.m_openCardTime == 0 then
                self.m_openCardTime = self.m_cbTimeLeave
            end 
            if self.isBagen then
               return
            end
            --开牌时间
             if self.m_cbTimeLeave == self.m_openCardTime then
            --发牌处理
                self:sendCard(true)
            elseif self.m_cbTimeLeave == self.m_openCardTime-4  then
            --显示点数
                self:showCard(true)
            elseif self.m_cbTimeLeave == self.m_openCardTime-10 then
            --金币处理
             self:showGoldMove()
            end
        end
