...
local GameViewLayer = appdf.req(appdf.GAME_SRC.."yule.brnn.src.views.layer.GameViewLayer")
local GameNewViewLayer = appdf.req(appdf.GAME_SRC.."yule.brnn.src.views.layer.GameNewViewLayer")
local GameFrame = appdf.req(appdf.GAME_SRC.."yule.brnn.src.models.GameFrame")
...
function GameLayer:onEventGameScene(cbGameStatus, dataBuffer)
	...
	self.m_bOnGame = true
    self._gameView.m_cbGameStatus = cbGameStatus --在这里设置游戏状态 保证的了数据的一致性
...
function GameLayer:onEventGameMessage(sub, dataBuffer)
    ...
    elseif sub == Game_CMD.SUB_S_TIME_STATUS then
        self:onTimeStatus(dataBuffer)
	end
end

function GameLayer:onTimeStatus(dataBuffer)
    local cmd_table = ExternalFun.read_netdata(Game_CMD.SUB_S_TimeStatus, dataBuffer)
    self._gameView.m_cbTimeLeave = cmd_table.btTime
    self._gameView.m_cbGameStatus = cmd_table.btStatus 
    self._gameView:updataDataTime( self._gameView.m_cbTimeLeave)  
end
