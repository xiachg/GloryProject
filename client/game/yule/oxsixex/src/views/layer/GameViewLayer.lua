...
local SetLayer = appdf.req(appdf.GAME_SRC.."yule.oxsixex.src.views.layer.SetLayer")
    ...
    for i = GameViewLayer.UiTag.eBtnBack,GameViewLayer.UiTag.eBtnTishi do
        ...
        if qBtn then
            if i == GameViewLayer.UiTag.eBtnBack then
                ...
                qBtn:setPosition(cc.p(56.0,display.height - 45.0))
            elseif i == GameViewLayer.UiTag.eBtnHelp then
                qBtn:setPosition(cc.p(1075.0,display.height - 45.0))
            elseif i == GameViewLayer.UiTag.eBtnSet then
                qBtn:setPosition(cc.p(1180.0,display.height - 45.0))
            elseif i == GameViewLayer.UiTag.eBtnChat then
                qBtn:setPosition(cc.p(1285.0,display.height - 45.0))
...
function GameViewLayer:menuClick(sender,type)
    if type == ccui.TouchEventType.ended then
        ...
        elseif tar == GameViewLayer.UiTag.eBtnHelp then
            print("GameViewLayer.UiTag.eBtnHelp")
...
function GameViewLayer:OnOpenCard()
    ...
    self._scene:PlaySound(GameViewLayer.RES_PATH.."sound/OPEN_CARD.wav")
end
...
function GameViewLayer:showGameEnd(t_data)
            ...
            if i ~= self.m_nChairID+1 then
                ...
                if t_hand ~= nil and t_hand[1] ~= 0 then
                    ...
                    if iValue <= 10 then
                        ...
                        if self.m_pUserItem_.cbGender == 1 then
                            strGender = "BOY"
                        end
                        local strFile = GameViewLayer.RES_PATH.."sound/" .. strGender.. "/ox_" .. iValue .. ".MP3"
                        self._scene:PlaySound(strFile)
                    end
                end
            end
        end
    ...
    self._scene:PlaySound(GameViewLayer.RES_PATH.."sound/GAME_END.wav")
    --清除数据
...
function GameViewLayer:showAdNiuK(bShow)
    ...
    self._scene:PlaySound(GameViewLayer.RES_PATH.."sound/CLICK_CARD.wav")
